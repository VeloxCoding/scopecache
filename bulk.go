...
func (s *store) replaceScopes(grouped map[string][]Item) (int, error) {
	...
	for scope, items := range grouped {
		// Validate the map KEY itself. Empty-slice batches
		...
		// path is safe (groupItemsByScope keys come from already-
		// validated item.Scope), but Gateway callers compose maps by
		// hand.
...
// input. Same lock + detach discipline as wipe.
